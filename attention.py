...
def scoremap_to_heatmap(score_map):
       # Apply the JET colormap to the score map
    heatmap_bgr = cv2.applyColorMap(np.uint8(255 * score_map), cv2.COLORMAP_JET)

    # Swap blue and red channels to reverse the color order
    heatmap_bgr = heatmap_bgr[..., [2, 1, 0]]  # Reverse the BGR to RGB order

    # Convert BGR to RGBA by adding an alpha channel
    heatmap_rgba = cv2.cvtColor(heatmap_bgr, cv2.COLOR_BGR2RGBA)

    # Adjust the overlay opacity (0 = completely transparent)
    heatmap_rgba[..., 3] = 60

    # Set all pixels with a score of 0 to be white and transparent
    heatmap_rgba[np.where(score_map == 0)] = (255, 255, 255, 0)
...
def main(args):
    ...
    scaled_tissue_mask = create_tissue_mask(wsi, wsi.get_best_level_for_downsample(1))

...
#     _, img_prepped = cv2.threshold(img_med, 127, 255, cv2.THRESH_BINARY) 
#     close = 4
...
def segment_tissue(img):
    ...
    # _, img_prepped = cv2.threshold(img_med, 127, 255, cv2.THRESH_BINARY)
    _, img_prepped = cv2.threshold(img_med, 180, 255, cv2.THRESH_BINARY)
    # _, img_prepped = cv2.threshold(img_med, 0, 255, cv2.THRESH_OTSU + cv2.THRESH_BINARY)
    kernel = np.ones((4, 4), np.uint8)
    img_prepped = cv2.morphologyEx(img_prepped, cv2.MORPH_CLOSE, kernel)

    contours, hierarchy = cv2.findContours(img_prepped, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
    return contours, hierarchy
...
def detect_foreground(contours, hierarchy):
    ...
    all_holes = [np.flatnonzero(hierarchy[:, 1] == cont_idx) for cont_idx in hierarchy_1]
    hole_contours = [[contours[idx] for idx in hole_ids] for hole_ids in all_holes]
    ...
    for foreground, holes in zip(foreground_contours, hole_contours):
        ...
        for hole_contour in holes:
            if len(hole_contour) < 3:
                continue
            hole = Polygon(np.squeeze(hole_contour))
            if not hole.is_valid or hole.area < min_area:
                continue
            poly = poly.difference(hole)
...
#     min_area = level_area / 5000000
#     # min_area = level_area / 5,000,000,000
...
def create_tissue_mask(wsi, seg_level):
    ...
    # Minimum surface area of tissue polygons (in pixels)
    # min_area = level_area / 5000000
    min_area = level_area / 50000000

    # Segment tissue to find contours and hierarchy
    contours, hierarchy = segment_tissue(img)
    print('length of contours', len(contours))
    print('length of hierarchy', len(hierarchy))

    # Detect foreground contours and holes
    foreground_contours, hole_contours = detect_foreground(contours, hierarchy)

    # Construct tissue mask polygon
    tissue_mask = construct_polygon(foreground_contours, hole_contours, min_area)

    # Scale the tissue mask polygon to be in the coordinate space of the slide's level 0
    scale_factor = wsi.level_downsamples[seg_level]
    tissue_mask_scaled = scale(tissue_mask, xfact=scale_factor, yfact=scale_factor, zfact=1.0, origin=(0, 0))
...
def load_encoder(backbone, checkpoint_file, use_imagenet_weights, device):
    ...
    if use_imagenet_weights:
        ...
        for k in list(state_dict.keys()):
            ...
            # if k.startswith("module.encoder_q") and not k.startswith( # for mocov2 resnet 50 encoder
            #     "module.encoder_q.fc" # for mocov2 resnet 50 encoder
            if k.startswith("module.base_encoder") and not k.startswith( # for mocov3 resnet 50 encoder
                "module.base_encoder.fc" # for mocov3 resnet 50 encoder
            ):
                # remove prefix from key names
                # state_dict[k[len("module.encoder_q.") :]] = state_dict[k] # for mocov2 resnet 50 encoder
                state_dict[k[len("module.base_encoder.") :]] = state_dict[k] # for mocov3 resnet 50 encoder
...
if __name__ == "__main__":
    ...
    seg_level = wsi.get_best_level_for_downsample(1)
